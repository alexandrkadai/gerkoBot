...
async function saveMessageToDatabase(chatId: string, message: Message, userId?: string) {
  ...
  try {
    ...
    // CRITICAL: Query only for ACTIVE (non-archived) chat_messages
    // This ensures we ignore archived rows and create new ones
    const { data: existingMessages, error: selectError } = await supabase
      .from('chat_messages')
      .select('id, chat_id, chat_history, archived')
      .eq('chat_id', chatId)
      .eq('archived', false) // ⭐ Only get non-archived rows
      .maybeSingle();
    ...
    if (!existingMessages) {
      // No active row exists (archived rows are ignored)
      // Create NEW row with archived = false
      ...
      const { data: insertData, error: insertError } = await (supabase.from('chat_messages') as any)
        .insert(insertPayload)
        .select()
        .single();

      if (insertError) {
        console.error(`❌ Failed to insert chat_messages row for chat ${chatId}:`, insertError);
        console.error('Insert error details:', insertError);
      } else {
        console.log(`💾 Created new chat_messages row for chat ${chatId} (id: ${insertData?.id})`);
      }
    } else {
      // Active row exists - append message to existing chat_history
      const currentHistory = (existingMessages as any).chat_history || [];
      const updatedHistory = [...currentHistory, message];

      const { error: updateError } = await (supabase.from('chat_messages') as any)
        .update({
          chat_history: updatedHistory,
          updated_at: new Date().toISOString(),
        })
        .eq('id', (existingMessages as any).id);

      if (updateError) {
        console.error(`❌ Failed to append message to chat ${chatId}:`, updateError);
      } else {
